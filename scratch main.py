...
m2Frame=Frame(re,bg='white')
m2Frame.place(x=111, y=180, width=1081,height=530)

current_step1 = 0
steps1 = []  # Store frames for each step


# Function to handle Next Button click
def next_step1():
    global current_step1
    if current_step1 < len(steps1) - 1:
        current_step1 += 1
        show_step()
...
def show_step1():
    ...
    for widget in m1Frame.winfo_children():
        widget.destroy()
    # Create and pack the current step frame
    frame2 = steps1[current_step]()
    frame2.pack(fill="both", expand=True)


# Function to create a generic TreeView frame
def create_treeview_frame2(title, col_defs1):
    def frame_func2():
        frameR = tk.Frame(m2Frame)
        # Title
        labelR = tk.Label(frameR, text=title, font=("Calibri", 20))
        labelR.pack(pady=10)

        # Treeview container (Frame for better layout flexibility)
        reFrame = Frame(frameR, bg='white')
        reFrame.pack(fill="both", expand=True, padx=20, pady=10)

        # Extract column names from col_defs
        columns1 = list(col_defs1.keys())

        # Create TreeView for the step
        table = ttk.Treeview(reFrame, columns=columns1, show='headings')
        table.pack(fill="both", expand=True)

        # Define columns
        for col, width in col_defs1.items():
            table.heading(col, text=col)  # Set the column heading
            table.column(col, width=width)  # Set the column width

        # Navigation buttons
        nav_frame2 = tk.Frame(frameR)
        nav_frame2.pack(pady=10)

        if current_step > 0:  # Show Previous button if not on the first step
            prev_button = ttk.Button(nav_frame2, text="Previous", command=previous_step)
            prev_button.pack(side="left", padx=5)

        if current_step < len(steps1) - 1:  # Show Next button if not on the last step
            next_button = ttk.Button(nav_frame2, text="Next", command=next_step)
            next_button.pack(side="right", padx=5)

        if current_step == len(steps1) - 1:  # Show Submit button on the last step
            submit_button = ttk.Button(nav_frame2, text="Submit", command=mFrame.quit)
            submit_button.pack(side="right", padx=5)
...
# Generate step functions dynamically based on step definitions
steps1 = [create_treeview_frame(title, col_defs1) for title, col_defs1 in step_definitions]
